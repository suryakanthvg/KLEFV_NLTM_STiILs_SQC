...
mindurspeechsegment = float(data["callDuration"])
...
with open("transcriptionfile.txt", "w") as file:
    ...
    for i in data['segments']:
        segmentType_count += 1
        print(i["segmentId"])
        Duration_of_speech_wavfile = Duration_of_speech_wavfile + (float(i["end"]) - float(i["start"]))
        if (i['segmentType'] == 'speech'):
           ...
           curspeechsegduration = float(i["end"]) - float(i["start"])
...
print('Duration (in seconds) of speech wavfile from transcription file header (callDuration)', float(data["callDuration"]))
